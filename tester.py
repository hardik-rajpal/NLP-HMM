...
class Tagger:
    ...
    def viterbi(self, Y):
        ...
        tags = np.asarray(['NN', 'LL', 'LS','LE'])

        A = trellis * (1 / np.tile(np.sum(trellis, 1).reshape((K, 1)), (1, K)))
        A = A.reshape((K,K))

        B = emmis * (1 / np.tile(np.sum(emmis, 0), (N, 1)))
        B = B.T
        B = B.reshape((K,N))
